...
function Doorlock() {
    ...
    const handleOpen = async () => {
        setIsOn(true);
        try {
            const response = await axios.post('http://43.203.119.177:8080/door/open'); //백엔드에서 제공하는 실제 주소로 대체
    ...
    const handleClose = async () => {
        setIsOn(false);
        try {
            const response = await axios.post('http://43.203.119.177:8080/door/close');
